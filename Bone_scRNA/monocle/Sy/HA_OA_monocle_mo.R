...
library(monocle)
library(dplyr)
setwd("/mnt/data7/yiyuan/Bone/test1/NEW_MPs/new/monocle/HA")
HA = readRDS("/mnt/data7/yiyuan/Bone/test1/NEW_MPs/new/monocle/sce_Sy_Mo_resubtype.final_HA.rds")
table(Idents(HA))

table(HA$sample)
head(HA@meta.data)

table(HA$celltype_Macrophages)
set.seed(1234)
...
mycds=sc_cds
save(mycds, file = "monocle_results_HA_MO.RData")

##使用clusters差异表达基因
diff.genes <- read.csv('/mnt/data7/yiyuan/Bone/test1/NEW_MPs/new/monocle/HA/diff_genes_wilcox.csv')
...
save(mycds, file = "monocle_results.HA.2.RData")
#State轨迹分布图
...
##合并作图
plot4 <- plot_cell_trajectory(mycds, color_by = "celltype_Macrophages")
ggsave("celltype_Macrophages.pdf", plot = plot4, width = 6, height = 5)
...
p3 <- plot_cell_trajectory(mycds, color_by = "celltype_Macrophages") +
  facet_wrap(~celltype_Macrophages, nrow = 2) +
  theme(
...
# 保存图片
ggsave("trajectory_facet_celltype_Macrophages.png", plot = p3, width = 25, height = 17)
...
save(mycds, file = "monocle_results_HA.3.RData")
...
load("monocle_results_HA.3.RData")
# 提取前 100 个基因名称
Time_genes  <- head(diff.genes,100)

# 保存基因名称到 CSV 文件
write.csv(Time_genes, "Time_diff_test100.1.csv", row.names = FALSE)
Time_genes=Time_genes$gene

# 绘制热图
p <- plot_pseudotime_heatmap(
  mycds[Time_genes, ],  # 使用有效的基因子集
  num_clusters = 8,
  show_rownames = TRUE,
  return_heatmap = TRUE
)
...
# 保存热图到文件
ggsave("Time_heatmapTop100.png", plot = p, width = 5, height = 10)
...
p2 <-plot_genes_in_pseudotime(cds_subset, color_by = "celltype_Macrophages")
...
save(mycds, file = "monocle_results_HA_Sy_Mo.RData")
...
library(monocle)
library(dplyr)
setwd("/mnt/data7/yiyuan/Bone/test1/NEW_MPs/new/monocle/OA")
OA = readRDS("/mnt/data7/yiyuan/Bone/test1/NEW_MPs/new/monocle/sce_Sy_Mo_resubtype.final_OA.rds")
table(Idents(OA))

table(OA$sample)
head(OA@meta.data)

table(OA$celltype_Macrophages)
set.seed(1234)
...
write.csv(top10, "top10_diff_genes_wilcox.csv", row.names = F)
mycds=sc_cds
save(mycds, file = "monocle_results_OA_MO.RData")

##使用clusters差异表达基因
diff.genes <- read.csv('/mnt/data7/yiyuan/Bone/test1/NEW_MPs/new/monocle/OA/diff_genes_wilcox.csv')
diff.genes <- subset(diff.genes,p_val_adj<0.01)$gene
mycds <- setOrderingFilter(mycds, diff.genes)
p1 <- plot_ordering_genes(mycds)
p1
...
save(mycds, file = "monocle_results.OA.2.RData")
#State轨迹分布图
...
save(mycds, file = "monocle_results_OA.3.RData")
...
save(mycds, file = "monocle_results_OA_Sy_Mo.RData")