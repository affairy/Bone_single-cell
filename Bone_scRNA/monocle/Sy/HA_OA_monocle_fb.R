...
library(monocle)
library(dplyr)
setwd("/mnt/data7/yiyuan/Bone/test1/Fibroblasts/new/monocle/HA")
HA = readRDS("/mnt/data7/yiyuan/Bone/test1/Fibroblasts/new/monocle/sce_Sy_Fb_resubtype.final_HA.rds")
table(Idents(HA))
...
mycds=sc_cds
save(mycds, file = "monocle_results_HA_Fb.RData")

##使用clusters差异表达基因
diff.genes <- read.csv('/mnt/data7/yiyuan/Bone/test1/Fibroblasts/new/monocle/HA/diff_genes_wilcox.csv')
...
p1 = plot_pseudotime_heatmap(mycds[sig_gene_names,], num_clusters=9,
...
clusters <- cutree(p1$tree_row,k=9)
...
#
save(mycds, file = "monocle_results_HA_Sy_Fb.RData")

load("/mnt/data7/yiyuan/Bone/test/Ch/Re_subtype/monocle/HA/monocle_results_HA_Ch.RData")
...
p5 <- plot_genes_in_pseudotime(mycds[s.genes,], color_by = "celltype")
plotc <- p3|p4|p5
ggsave("genes_visual_celltype_SDC2.png", plot = plotc, width = 30, height = 8)
...
library(monocle)
library(dplyr)
setwd("/mnt/data7/yiyuan/Bone/test1/Fibroblasts/new/monocle/OA1")
OA = readRDS("/mnt/data7/yiyuan/Bone/test1/Fibroblasts/new/monocle/sce_Sy_Fb_resubtype.final_OA.rds")
table(Idents(OA))

table(OA$sample)
head(OA@meta.data)

set.seed(123)  # 固定随机数种子，确保结果可重复
scRNA_subset <- subset(OA, downsample = 2500)

OA=scRNA_subset
saveRDS(OA,file="sce_scRNA2500.rds")
...
write.csv(top10, "top10_diff_genes_wilcox.csv", row.names = F)
mycds=sc_cds
save(mycds, file = "monocle_results_OA_Fb.RData")

##使用clusters差异表达基因
diff.genes <- read.csv('/mnt/data7/yiyuan/Bone/test1/Fibroblasts/new/monocle/OA1/diff_genes_wilcox.csv')
diff.genes <- subset(diff.genes,p_val_adj<0.01)$gene
mycds <- setOrderingFilter(mycds, diff.genes)
p1 <- plot_ordering_genes(mycds)
p1
...
plot_ordering_genes(mycds)
dev.off()
load("monocle_results_OA_Fb.RData")
##########################使用disp.genes开展后续分析
#降维
mycds <- reduceDimension(mycds, max_components = 2, method = 'DDRTree')
#排序
...
#ggsave("trajectory_facet_FPR1.png", plot = p2, width = 15, height = 8)
#
save(mycds, file = "monocle_results_OA_Sy_Fb.RData")
